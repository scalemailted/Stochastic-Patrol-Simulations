...
// Array to store the green celled adversaries
let adversaries = [];

// Set the number of adversaries
const numAdversaries = 10;

// Create the adversaries
for (let i = 0; i < numAdversaries; i++) {
  let adversaryX = Math.floor(Math.random() * targetAreaWidth / size) * size;
  let adversaryY = Math.floor(Math.random() * targetAreaHeight / size) * size;
  adversaries.push({ x: adversaryX, y: adversaryY, discovered: false });
}
...
// Update the canvas to draw the adversaries
const drawCanvas = () => {
...
const drawAdversaries = () => {
  ctx.fillStyle = "green";
  adversaries.forEach(adversary => {
    ctx.fillRect(adversary.x, adversary.y, size, size);
  });
};

/*
const updateAdversaries = currentLocation => {
  adversaries.forEach(adversary => {
    if (!adversary.discovered) {
      adversary.x += Math.floor(Math.random() * 3 - 1) * size;
      adversary.y += Math.floor(Math.random() * 3 - 1) * size;

      if (adversary.x >= targetAreaWidth) {
        adversary.x = targetAreaWidth - size;
      } else if (adversary.x < 0) {
        adversary.x = 0;
      }
      if (adversary.y >= targetAreaHeight) {
        adversary.y = targetAreaHeight - size;
      } else if (adversary.y < 0) {
        adversary.y = 0;
      }

      if (adversary.x === currentLocation.x && adversary.y === currentLocation.y) {
        adversary.discovered = true;
      }
    }
  });
};*/
...
// Update the state of the quadrotor and the canvas
const move = () => {
  ...
  updateAdversaries(currentLocation);
  drawCanvas();
  ...
  const printEnemies = () => {
    const activeAdversaries = adversaries.filter(adversary => adversary.discovered==false);
    document.getElementById("enemies").innerHTML = `Enemies: ${activeAdversaries.length}`;
  };
